...
#for v in [27]:
    ...
    #for h in range(17, 40):
        ...
        while not done and timeit.default_timer() - init_ds_time < 1000:
            critical_neighbors = get_critical_neighbors(criticalNode(current_sol), best_known_size)

            selection = [current_sol[random.randint(0, k - 1)],
                         critical_neighbors[random.randint(0, len(critical_neighbors) - 1)]]
                ...
                if random.random() <= p_sa:
                    ...
                    for j in range(0, k):
                        if current_sol[j] == selection[1]:
                            current_sol[j] = selection[0]
                    if not is_local_opt:
                        interchange_result = interchange(current_sol, current_size)
                        current_sol = interchange_result[0]
                        current_size = interchange_result[1]
                        is_local_opt = True
